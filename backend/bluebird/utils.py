...
def prepare_act_data(request, package):
    ...
    data['by_plan'] = json.loads(request.data.get('by_plan'))
    data['by_phys'] = json.loads(request.data.get('by_phys'))
    data['phys_data'] = json.loads(request.data.get('phys_data'))
    data['by_jur'] = json.loads(request.data.get('by_jur'))
    data['jur_data'] = json.loads(request.data.get('jur_data'))
    data['address'] = json.loads(request.data.get('address'))
    data['exam_descr'] = json.loads(request.data.get('exam_descr'))
    data['evidence'] = json.loads(request.data.get('evidence'))
    data['add_info'] = json.loads(request.data.get('add_info'))
    data['exam_result'] = json.loads(request.data.get('exam_result'))
    print('all fine')
    data['photos'] = list()
